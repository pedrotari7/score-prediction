...
export default function Navbar({ loading, setLoading }: { loading: boolean; setLoading: (loading: boolean) => void }) {
	...
	return (
		<>
			...
			<nav className='glass-panel fixed inset-x-1 bottom-2 z-20 flex h-14 select-none items-center justify-around rounded-2xl px-1 shadow-glass lg:hidden'>
				{indicatorRect && (
					<div
					...
					primaryNav.map((item, index) => {
						...
						return (
							<button
								...
								onClick={() => updateRoute(item.info)}
								className={classNames(
									'relative z-10 flex flex-1 flex-col items-center gap-0.5 rounded-xl px-1 py-1.5 text-[11px] font-semibold transition-colors duration-200',
									active
										? 'text-white'
