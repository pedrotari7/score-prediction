import { Lineup, LineupPlayer, LineupPlayers, Player } from '../../interfaces/main';
...
const GameLineup = ({ lineups }: { lineups: Lineup[] }) => {
	...
	const LineupField = ({
		...
		const PlayerPosition = ({ player }: { player: LineupPlayer }) => {
				...
				<div className="absolute w-full h-full">
					<div className="flex flex-col sm:flex-row w-full h-full justify-center items-center">
						<div className="sm:w-1/2 w-full h-1/2 sm:h-full flex flex-col sm:flex-row">
							{Object.values(homeSections).map(sectionXI)}
						</div>
						<div className="sm:w-1/2 w-full h-1/2 sm:h-full flex flex-col sm:flex-row">
							{Object.values(awaySections).reverse().map(sectionXI)}
						</div>
					</div>
				</div>
			</div>
