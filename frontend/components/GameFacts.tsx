...
const TimeElapsed = ({ event }: { event: Event }) => (
	<div className='mx-2 flex w-5 flex-col items-center justify-center'>
		<span className='font-bold'>{event.time.elapsed}&apos;</span>
		{event.time.extra !== null && <span className='text-xs font-bold text-gray-400'>+{event.time.extra}</span>}
	</div>
);
...
const GameEvent = ({ event, game, players }: { event: Event; game: Fixture; players: PlayersMap }) => {
				...
				<svg width='24px' height='21px' viewBox='0 0 24 21' version='1.1' xmlns='http://www.w3.org/2000/svg'>
					...
					<g id='Page-1' stroke='none' strokeWidth='1' fill='none'>
						<g id='icVar' fill='white'>
							<polygon id='Fill-1' points='11.00702 9.18445 12.57398 9.18445 11.79102 6.7395'></polygon>
							<path
	...
	const events = extraInfo.events?.map((event, idx) => {
		...
		return (
			<div key={idx} className='w-full xl:w-1/3'>
				{shouldAddHTScore && isGameFinished(game) && <Score score={game.score.halftime} label='HT' />}
				{shouldAddFTScore && isGameFinished(game) && <Score score={game.score.fulltime} label='FT' />}
				<GameEvent event={event} game={game} players={players} />
				{shouldAddHTScore && !isGameFinished(game) && <Score score={game.score.halftime} label='HT' />}
				{shouldAddFTScore && !isGameFinished(game) && <Score score={game.score.fulltime} label='FT' />}
			</div>
		);
