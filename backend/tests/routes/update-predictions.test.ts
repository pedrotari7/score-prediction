...
describe('POST /update-predictions', () => {
  it('saves a valid prediction for a future game', async () => {
    ...
    const res = await request(app)
      ...
      .send({ gameId: 1001, prediction: { home: 2, away: 1 } });
  ...
  it('rejects prediction for a past game', async () => {
    ...
    const res = await request(app)
      .post(`/update-predictions?competition=${COMPETITION}`)
      .set('Authorization', userToken)
      .send({ gameId: 2001, prediction: { home: 1, away: 0 } });

    expect(res.status).toBe(403);
    expect(res.body.error).toBe('Forbidden');
  });

  it('always saves prediction under the authenticated user uid, ignoring any uid in body', async () => {
    ...
    const res = await request(app)
      .post(`/update-predictions?competition=${COMPETITION}`)
      .set('Authorization', userToken)
      .send({ gameId: 1002, prediction: { home: 1, away: 0 } });
    ...
    const doc = await getDb().collection(COMPETITION).doc('predictions').get();
    const data = doc.data()!;
    expect(data['1002'][USER_UID]).toEqual({ home: 1, away: 0 });
  });
  ...
  it('rejects invalid score values', async () => {
    ...
    const res = await request(app)
      .post(`/update-predictions?competition=${COMPETITION}`)
      .set('Authorization', userToken)
      .send({ gameId: 1003, prediction: { home: -1, away: 0 } });
  ...
  it('rejects score over 99', async () => {
    ...
    const res = await request(app)
      .post(`/update-predictions?competition=${COMPETITION}`)
      .set('Authorization', userToken)
      .send({ gameId: 1004, prediction: { home: 0, away: 100 } });
  ...
  it('rejects non-integer scores', async () => {
    ...
    const res = await request(app)
      .post(`/update-predictions?competition=${COMPETITION}`)
      .set('Authorization', userToken)
      .send({ gameId: 1005, prediction: { home: 1.5, away: 0 } });
  ...
  it('allows null scores (clearing a prediction)', async () => {
    ...
    const res = await request(app)
      .post(`/update-predictions?competition=${COMPETITION}`)
      .set('Authorization', userToken)
      .send({ gameId: 1006, prediction: { home: null, away: null } });
